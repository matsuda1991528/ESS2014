...
#define OPERATE_FINISH 100         //走査終了時間[sec]
#define TARGET_X 1500                 //x軸への目標移動距離[mm]
...
#define MAX 9999
#define MIN 0
#define CREATE_ACROSS 150 //irobot createの半径[mm]
...
int goVirticalWay(){
	...
	if(fp1){
		while(1){
			...
			else if(abs(sum_data.x - temp_x) >= target_distance){
				directDrive(0, 0);
				break;
			}
			else{
